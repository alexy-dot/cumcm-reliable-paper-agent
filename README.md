...
| 建模与复核 | 独立输出比对；分组验证与公平基线；供货预测支持域、整条曲线覆盖复核 |
...
[旧盲测复核](benchmarks/reports/profilometer-2020d-legacy-audit.json)：2020 D旧稿虽自评93分，仍缺少完整轮廓修正；全曲线复算发现局部重合指标掩盖了大段偏离。该稿不作为完整盲测通过证据。
