...
def compose(run):
    ...
    section("一、问题重述", [
        text("回焊炉通过不同温区加热与冷却印刷电路板，使电子元件完成焊接。炉温曲线指焊接区域中心随时间变化的温度；合适的曲线既要达到焊接温度，也要避免升降温过快或高温停留过久。题目提供一次实验的中心温度记录，要求建立可用于调整温区设定和传送速度的数学模型[1]。"),
        text(r"题面给出11个小温区，单区长度30.5 cm，相邻间隙5 cm，炉前与炉后各25 cm。因此总行程为$25+11\times30.5+10\times5+25=435.5$ cm，第$i$区起点为$25+(i-1)\times35.5$ cm。传送速度$v$以cm/min给出，内部统一为$v/60$ cm/s，位置与时间关系为$x=vt/60$。"),
        ...
        text("150—190℃持续时间只统计上升过程；217℃以上持续时间包括上升和下降两段；面积指标以217℃水平线为基线且止于峰值。所有结果表均按显示精度列出，计算保留完整数值。"),
        table([["制程指标", "下限", "上限", "单位"], ["温度上升斜率", "0", "3", "℃/s"], ["温度下降斜率", "−3", "0", "℃/s"],
               ["上升过程中150—190℃时间", "60", "120", "s"], ["高于217℃的总时间", "40", "90", "s"], ["峰值温度", "240", "250", "℃"]])], True)
    ...
    section("附录", [
        text("附录A：程序与支撑文件清单。以下文件共同构成本次训练的复现入口；正式提交时还须按当年要求附上全部完整源码及AI工具使用详情。"),
        ...
        text("复现依次执行prepare.py、calibrate.py、solve.py、independent.py、sensitivity.py、plots.py与paper.py，最后通过render_paper.py的TeX后端编译。所有输入从冻结sources目录读取，result.csv位于artifacts；源码和依赖声明随项目提供。")])
