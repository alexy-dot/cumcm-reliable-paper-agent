...
- 算法：先明确题目瓶颈，再选择能够解释和解决它的方法。若声称组合或升级有益，用相同数据、约束、起点策略及计算预算比较基础法、增加的组件和组合，报告可行解数量、目标分布及代价；把独立复核成本单列。共同基准起点不算多个独立初值。若未观察到增益，保留简洁方法，不把算法数量当作创新。
- 文字：问题分析说明“为何这样做”，建模段给出完整目标与约束，求解段交代实际起点、不可行解处理、停止条件和选解依据，结果段回答题目。围绕读者无法复算或无法判断的缺口补写，不以固定字数或页数代替内容判断。摘要突出每问的贡献与关键数字，避免工程日志式叙述。
